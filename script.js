...
const generateLetter = async (generatePdfFlag = false) => {
    ...
    content = content.replace(/\*\*(.*?)\*\*/g, '</w:t></w:r><w:r><w:rPr><w:b/><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/><w:i/></w:rPr><w:t>$1</w:t></w:r><w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/><w:i/></w:rPr><w:t>');
    
    content = content.replace(/\*(.*?)\*/g, '</w:t></w:r><w:r><w:rPr><w:i/><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/></w:rPr><w:t>$1</w:t></w:r><w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/><w:i/></w:rPr><w:t>');
    
    // Обрабатываем переводы строк - КАЖДЫЙ перенос становится новым абзацем
    ...
    return paragraphs.join('</w:t></w:r></w:p><w:p><w:pPr><w:spacing w:before="0" w:after="0" w:line="300" w:lineRule="auto"/><w:jc w:val="both"/><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/><w:i/></w:rPr></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="24"/><w:i/></w:rPr><w:t>');
        ...
        // ИСПРАВЛЕННАЯ ЧАСТЬ: Генерация файла с правильным MIME-типом
        ...
        // Улучшенное скачивание для всех устройств
        if (navigator.msSaveBlob) {
            // Для Internet Explorer
            navigator.msSaveBlob(docxBlob, fileName);
        } else {
...
// ДОБАВЬТЕ эти методы в раздел methods
handleTextareaKeydown(event) {
    // Enter работает по умолчанию в textarea, этот метод оставлен для возможных будущих улучшений
    if (event.key === 'Tab') {
...
// Функция преобразования простого форматирования в HTML
formatSimpleText(text) {
    ...
    updateContent(event) {
        ...
        // Сохраняем HTML как есть, но нормализуем переводы строк
        let content = editable.innerHTML;
        
        // Заменяем множественные пробелы и нормализуем переводы строк
        content = content.replace(/<br\s*\/?>/gi, '<br>');
        
        formData.value.letter.content = content;
    },
